var namespacePattern = /^\s*@namespace\s+url\(\"?http:\/\/www.w3.org\/1999\/xhtml\"?\);?\s*$/;

function getDatabase(ready, error) {
	var d = openDatabase('stylish', '', 'Stylish Styles', 5*1024*1024);
	if (d.version == "1.0" || d.version == "") {
		dbV11(d, error, ready);
	} else if (d.version == "1.1") {
		dbV12(d, error, ready);
	} else if (d.version == "1.2") {
		dbV13(d, error, ready);
	} else {
	  ready(d);
	}
	...
	getDatabase(function(db) {
		db.readTransaction(function (t) {
			if ("matchUrl" in options) {
				...
				var sql = "SELECT DISTINCT s.style_id FROM sections s INNER JOIN section_meta sm ON sm.section_id = s.id WHERE (sm.name = 'url' and sm.value = ?) OR (sm.name = 'url-prefix' AND ? LIKE (sm.value || '%'))";
				var matchParams = [];
				var domains = getDomains(options.matchUrl);
				matchParams = matchParams.concat([options.matchUrl, options.matchUrl]).concat(domains);
				var domainClause = "";
				if (domains.length == 1) {
